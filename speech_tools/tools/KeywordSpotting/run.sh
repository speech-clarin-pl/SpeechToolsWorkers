...
if [ $# -ne 3 ]; then
  echo "Usage: ./run.sh <input-wav> <keywords> <out-kws>"
  echo ""
  echo "Options:"
  echo "    --model_name"
  exit 1
...
lattice-scale --lm-scale=12 "ark:gunzip -c tdnn/decode_rs/lat.*.gz|" ark,t:- | ./utils/int2sym.pl -f 3 tree/graph/words.txt > lattice.txt

./kws/KaldiLattice lattice.txt $kws kws/lexicon.txt > $out
