...
# This script produces phoneme CTM files from a decoding directory that has lattices
# present.
...
if [ $stage -le 0 ]; then
  ...
  if [ -f $lang/phones/word_boundary.int ]; then
    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/get_phoneme_ctm.LMWT.log \
      ...
      $filter_cmd '>' $dir/score_LMWT/${name}_phoneme.ctm || exit 1;
