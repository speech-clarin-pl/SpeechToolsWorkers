...
if [ $stage -le 0 ]; then
  if [ -f $lang/phones/word_boundary.int ]; then
    $cmd JOB=1:$nj $dir/log/get_phonectm.JOB.log \
      ...
      gzip -c '>' $dir/phonectm.JOB.gz || exit 1
  else
...
if [ $stage -le 1 ]; then
  if [ -f $data/segments ] && $use_segments; then
    ...
    [ ! -f $f ] && echo "$0: expecting file $f to exist" && exit 1;
    for n in `seq $nj`; do gunzip -c $dir/phonectm.$n.gz; done | \
      utils/convert_ctm.pl $data/segments $data/reco2file_and_channel > $dir/phonectm || exit 1;
  else
    for n in `seq $nj`; do gunzip -c $dir/phonectm.$n.gz; done > $dir/phonectm || exit 1;
  fi
  rm $dir/phonectm.*.gz
fi
